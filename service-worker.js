// gate-da-tracker Service Worker v1.0
// Caches the entire app for full offline support

const CACHE_NAME = 'gate-da-tracker-v1';
const ASSETS_TO_CACHE = [
  '/gate-da-tracker/',
  '/gate-da-tracker/index.html',
  '/gate-da-tracker/StudyForge_Tracker_V3.html',
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      console.log('[gate-da-tracker SW] Caching app shell');
      // addAll fails if any single request fails, so we cache individually
      return Promise.allSettled(
        ASSETS_TO_CACHE.map(url =>
          cache.add(url).catch(err =>
            console.warn('[gate-da-tracker SW] Failed to cache:', url, err)
          )
        )
      );
    ...
    caches.keys().then(keys =>
      Promise.all(
        keys
          ...
          .map(key => {
            console.log('[gate-da-tracker SW] Deleting old cache:', key);
...
self.addEventListener('fetch', event => {
  ...
  // For Google Fonts and app assets: cache-first
    ...
    url.pathname.includes('gate-da-tracker')
  ) {
    event.respondWith(
